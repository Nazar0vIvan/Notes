...
**Схемой (scheme) \(\mathcal{S}\)** отношения \(R\) называется **кортеж** пар атрибут-домен \((A_i,D_{A_i})\):

$$\mathcal{S}=\Big(⟨A_1,D_{A_1}⟩,⟨A_2,D_{A_2}⟩,...⟨A_n,D_{A_n}⟩\Big)$$

Например,

$$
\mathcal{S}_1=\Big(⟨name,string⟩,⟨surname,string⟩,⟨age,int⟩\Big)\ \ \ \ \ \\
\mathcal{S}_2=\Big(⟨color,COLOR⟩,⟨length,float⟩,⟨width,float⟩\Big)
$$

Иногда в схему \(\mathcal{S}\) отношения \(R\) включают ещё и имя самого отношения \(R\) и обозначают следующим образом:
...
$$
\begin{array}{l}
STUDENT(name:string,\ surname: string,\ age: int,\ rating: float)\\[0.5em]
...
\mathcal{B}=\{(f_1, f_2,...f_n):\ i=\overline{1,n}\}\\[0.5em]
...
<img src="/operators.svg" style="width: 700px"/>

$$
\begin{array}{l}
\sigma_{condition}(R)=\{r\in R:condition(R)\}\\[0.5em]
condition: R \rightarrow B\\[0.5em]
B=\{0,1\}
\end{array}
$$
